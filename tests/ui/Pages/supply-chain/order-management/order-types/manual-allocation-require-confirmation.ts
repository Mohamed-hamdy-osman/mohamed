 ...
 * Variant: Manual Allocation + Require Confirmation (selects a confirming User).
 */
export class manualAllocationRequireConfirmationOrderTypePage {
  readonly page: Page;
  ...
  readonly requireConfirmationRadio: Locator;
  ...
  constructor(page: Page) {
    ...
    this.requireConfirmationRadio = page.locator('#requireConfirmation');
    this.userDropdown = this.dialog.getByRole('combobox', { name: 'Select User' });
  ...
  /** Generates a name like "Man Alloc 482" */
  static generateOrderName(prefix = 'Man Alloc'): string {
  ...
  /** Checks "Require Confirmation" (instead of Auto) */
  async selectRequireConfirmation() {
    await expect(this.requireConfirmationRadio).toBeVisible();
    await this.requireConfirmationRadio.click({ force: true });
  }

  /** Search for and select a confirming user, e.g. "Super Admin" */
  ...
  async selectManualAllocation() {
    await this.manualAllocationRadio.click();
  }
  ...
  /** Full "manual allocation + require confirmation" happy-path flow, returns the generated order name */
  async createManualAllocationOrderType(branchName = 'Cairo Branch'): Promise<string> {
    const orderName = manualAllocationRequireConfirmationOrderTypePage.generateOrderName();
    ...
    await this.selectRequireConfirmation();
    await this.selectUser('Super Admin');
    await this.selectFirstMoveOrderTransaction();
    await this.selectManualAllocation();
