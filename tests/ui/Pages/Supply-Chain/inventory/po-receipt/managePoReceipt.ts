...
export class ManagePOReceiptPage {
  ...
  readonly PoReceipt_menu: Locator;
  ...
  constructor(page: Page) {
    ...
    this.PoReceipt_menu = page.getByRole('link', {
      name: 'Po Receipt',
      exact: true
    });
  ...
  async navigateToPOReceipt() {
    ...
    await Promise.all([
      this.page.waitForURL(/po-receipt/),
      this.PoReceipt_menu.click()
    ]);
  }

  async verifyNavigationToPOReceipt() {

    await expect(this.page).toHaveURL(/po-receipt/);
