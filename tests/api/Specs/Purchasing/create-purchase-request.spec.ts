...
test('Verify user can create purchase request', async ({ request }) => {
  ...
  // Step 2: Add Purchase Request Line
  const addPurchaseRequestLine = new AddPurchaseRequestLine(request);

  const addLineResponse = await addPurchaseRequestLine.addPurchaseRequestLine(
    purchaseRequestId
  );

  expect([200, 202]).toContain(addLineResponse.status());
